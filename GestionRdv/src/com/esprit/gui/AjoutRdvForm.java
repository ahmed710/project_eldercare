...
public class AjoutRdvForm extends Form {
    ...
    private Picker date;
    private ComboBox<User> medecins;
    ...
    private void OnGui() {
        ...
        medecins = new ComboBox<User>();
        ServiceUser su = new ServiceUser();
        List<User> medecinsList = su.afficher();

        // Ajouter les noms des médecins à la ComboBox
        for (User medecin : medecinsList) {
            medecins.addItem(medecin);
        }

        btnAjout = new Button("Ajouter");
        this.addAll(date, medecins, btnAjout);
    }

    private void addActions() {
        btnAjout.addActionListener((evt) -> {
            if (date.getText().isEmpty() || medecins.getSelectedItem().toString().isEmpty()) {
                Dialog.show("Alerte", "Veillez remplir tous les champs", "OK", null);
            } else {
                ServiceRdv sr = new ServiceRdv();
                if (sr.ajouter(new RendezVous(date.getDate().toString(), medecins.getSelectedItem().getNom()),date.getDate())) {
